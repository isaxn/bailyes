...
Contoh mengirim tombol:

```js
const { bx } = require("@isaxn/bailyes");

await bx.button(bot.sock, "6281234567890@s.whatsapp.net", {
  title: "Menu",
  body: "Silakan pilih:",
  footer: "Isaxn Bailyes",
  buttons: [
    { text: "Cek profil", id: "profile" },
    { text: "Bantuan", id: "help" }
  ]
});
```

`buttons` wajib berupa array yang berisi minimal satu objek `{ text, id }`.

Contoh mengirim tombol dari dalam command:
...
bot.command("menu", async (ctx) => {
  await bx.button(ctx.sock, ctx.chat, {
    body: "Pilih menu:",
    buttons: [
      { text: "Produk", id: "products" },
      { text: "Kontak", id: "contact" }
    ],
...
Event `close` mengirim objek ringkas `{ statusCode, reason, message }`. Untuk kode `515`, nilai `message` berisi `Oops, koneksi terputus. Mencoba menyambungkan ulang...`, sehingga cukup tampilkan:

```js
bot.onFramework("close", ({ message }) => console.log(message));
```

Error mentah masih tersedia melalui properti `error` bila benar-benar diperlukan untuk debugging, tetapi tidak ikut tercetak saat objek `close` ditampilkan dengan `console.log`.
...
- **Log koneksi kode `515` diringkas** menjadi pesan yang mudah dibaca tanpa memenuhi console dengan stack error.
- **Reconnect lebih aman** — event socket lama diabaikan dan timer reconnect dibersihkan ketika bot dihentikan.
...
- Ditambahkan `bx` — cara panggil satu fungsi yang ringkas, plus fitur baru `bx.livePhoto` dan `bx.liveThumbnail`.
